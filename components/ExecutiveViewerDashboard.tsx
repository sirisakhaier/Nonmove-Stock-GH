  ...
  return (
    <div className="space-y-8">
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              <h2 className="text-xl font-black text-slate-900 dark:text-white">
                วิเคราะห์ภาพรวมสต๊อกไม่เคลื่อนไหว (Non-Move Analysis)
              </h2>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
              วิเคราะห์ตามช่วงวัน (Period), จำนวนชิ้น (QTY), รายการ (SKU), มูลค่า (Amount) และแนวโน้มรายวัน
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="h-64 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={periodBreakdown} margin={{ top: 10, right: 10, left: -10, bottom: 20 }}>
                ...
                <Tooltip
                  formatter={(val: any) => [`฿${Number(val).toLocaleString("th-TH")}`, "มูลค่าสต๊อกรวม (Amount)"]}
        ...
        {/* Chart 2: Daily Trend Progression */}
        <div className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 shadow-sm space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-base font-bold text-slate-900 dark:text-white">
            ...
            {trendData?.historicalSnapshots?.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={trendData.historicalSnapshots} margin={{ top: 10, right: 10, left: -10, bottom: 20 }}>
                  <defs>
                    <linearGradient id="colorViewerTrendV3" x1="0" y1="0" x2="0" y2="1">
                  ...
                  />
                  <Area type="monotone" dataKey="totalStockValue" stroke="#6366f1" strokeWidth={3} fillOpacity={1} fill="url(#colorViewerTrendV3)" name="totalStockValue" />
                </AreaChart>
              </ResponsiveContainer>
            ) : (
